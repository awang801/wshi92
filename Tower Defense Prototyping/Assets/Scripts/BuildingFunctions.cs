...
public class BuildingFunctions : MonoBehaviour {
	...
	void Awake()
	{
		...
		UIBuildFX = (AudioClip)(Resources.Load ("UIButtonclick", typeof(AudioClip)));
	...
	public void BuildWall()
	{
		if (isBuilding == true) { 
			...
			{
				mFunc.SelHighlight = ((GameObject)(Instantiate(Resources.Load("SelectionHighlight")))); //Creates green selection box
			}
			sourceSFX.PlayOneShot(UIBuildFX);
	...
	void CheckButtons()
	{
		...
		if (Input.GetButtonDown ("Cancel")) { //If ESC is pressed
		...
		} else if (Input.GetButtonDown ("Build")) { //If B is pressed, Enter Build mode
			
			buildButton.BuildToggle ();

		} else if (Input.GetButtonDown ("Z")) { //If Z is pressed, and Building Mode is enabled, -- Build wall

			BuildWall ();

		} else if (Input.GetButtonDown ("X")) { //If X is pressed, and Building Mode is enabled, -- Build basic tower
